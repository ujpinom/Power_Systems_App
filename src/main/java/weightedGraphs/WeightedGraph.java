...
public class WeightedGraph<V> extends UnweightedGraph<V> {
  ...
  public WeightEdeges crearEdge(int u, int v, double weight) {
    ...
    if (v < 0 || v > getSize() - 1) throw new IllegalArgumentException("No existe tal indice" + v);
  ...
  public void printWeightEdges() {
    ...
    for (int i = 0; i < vertices.size(); i++) {
      ...
      for (Edges e : vecinos.get(i)) {
        System.out.print(
            "("
                + getVertex(((WeightEdeges) e).getU())
                + ", "
                + getVertex(((WeightEdeges) e).getV())
                + ", "
                + ((WeightEdeges) e).getWeight()
                + ") ");
      }
      System.out.println();
    }
