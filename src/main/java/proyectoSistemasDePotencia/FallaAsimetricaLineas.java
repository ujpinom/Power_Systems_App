...
public class FallaAsimetricaLineas {
	...
	public void calculoVectorVoltajes() {
		...
		vA1Imaginario = elementoFallado.getTensionLineaPuntoFalla()
				+ matrizSecuencia1[matrizSecuencia0.length - 1][matrizSecuencia0.length - 1] * corrienteFalla;
	...
	public void calculoCorrientesFalla() {
		...
		for (int i = 0; i < generador.size(); i++) {
			...
			double iA1 = (-1) * (generador.get(i).getBarra().getVoltajePrefalla()
					- vectorVoltajesSecuencia1.get(indexBarraGe - 1)) / generador.get(i).getImpedanciaZ1();
