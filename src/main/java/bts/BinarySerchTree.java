...
public class BinarySerchTree<E extends Comparable<E>> extends AbstractTree<E> {
    ...
    if (root == null) {
      ...
      while (actual != null) {
        ...
        if (e.compareTo(actual.elemento) < 0) {
          ...
          return false; // no se puede crear el nuevo nodo porque el valor ya es ingual a un nodo
          // existente
        }
      }
      ...
      } else padre.izquierda = crearNuevoNodo(e);
    }
  ...
  public int helper(TreeNode<E> root) {
    ...
    if (root == null) {
      return 0;
    } else return 1 + Math.max(helper(root.izquierda), helper(root.derecha));
  }
