...
public class FallaTrifasicaLinea {
  ...
  public FallaTrifasicaLinea(
    ...
    double[][] zBarrafinal =
        crz.inserccionEntreBarras(
            matrizInserccionEntreDosBarras,
  ...
  public void setContribuciones() {
    ...
    for (int i = 0; i < barras.size(); i++) {
      ...
      if (barras.get(i) == elementoFallado.getBarra1()
          || barras.get(i) == elementoFallado.getBarra2()) {

        double contribuccion =
            (barras.get(i).getVoltajePosFallaFaseA() - voltajeBarraImaginaria)
    ...
    for (int i = 0; i < zBarra.length - 1; i++) {
      ...
      double elemento =
          barras.get(i + 1).getVoltajePrefalla()
              - (zBarra[i][zBarra.length - 1]
                      / (zBarra[zBarra.length - 1][zBarra.length - 1]
                          + elementoFallado.getImpedanciaFalla()))
                  * barras.get(i + 1).getVoltajePrefalla();
  ...
  public void setCorrienteFallaElementoFallado() {
    ...
    voltajeBarraImaginaria =
        elementoFallado.getTensionLineaPuntoFalla()
            + corrientePuntoFalla * zBarra[zBarra.length - 1][zBarra.length - 1];
