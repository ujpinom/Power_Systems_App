      ...
      for (int i = k + 1; i < n; i++) {
        ...
        for (int s = 0; s < n; s++) {
          try {
            B[i][s] =
                Complejo.resta(
                    B[i][s], Complejo.cociente(Complejo.producto(A[i][k], B[k][s]), A[k][k]));
        ...
        for (int j = k + 1; j < n; j++) {
          try {
            A[i][j] =
                Complejo.resta(
                    A[i][j], Complejo.cociente(Complejo.producto(A[i][k], A[k][j]), A[k][k]));
          } catch (ExcepcionDivideCero e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
          }
      ...
      for (int i = n - 2; i >= 0; i--) {
        ...
        for (int k = n - 1; k > i; k--) {
          try {
            C[i][s] =
                Complejo.resta(
                    C[i][s], Complejo.cociente(Complejo.producto(A[i][k], C[k][s]), A[i][i]));
          } catch (ExcepcionDivideCero e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
          }
