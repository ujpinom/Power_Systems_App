...
public class LineForm extends AbstractForm<Lineas> {
    ...
    public TitledPane render(Lineas linea) {
        ...
        linea.addPropertyChangeListener(evt -> {
            if ("nombreLinea".equals(evt.getPropertyName())) {
                javafx.application.Platform.runLater(() -> txtLogico.setText((String) evt.getNewValue()));
            }
        });
        ...
        txtZ1.focusedProperty().addListener((obs, oldVal, newVal) -> {
            if (!newVal)
                actualizarZ1(linea, txtZ1);
        });
        addField("Z1 (p.u.):", 2, txtZ1);

        // --- 3. IMPEDANCIA Z2 ---
        TextField txtZ2 = new TextField(String.valueOf(linea.getimpedanciaLineaZ2()));
        txtZ2.setOnAction(e -> actualizarZ2(linea, txtZ2));
        txtZ2.focusedProperty().addListener((obs, oldVal, newVal) -> {
            if (!newVal)
                actualizarZ2(linea, txtZ2);
        });
        addField("Z2 (p.u.):", 3, txtZ2);

        // --- 4. IMPEDANCIA Z0 ---
        TextField txtZ0 = new TextField(String.valueOf(linea.getimpedanciaLineaZ0()));
        txtZ0.setOnAction(e -> actualizarZ0(linea, txtZ0));
        txtZ0.focusedProperty().addListener((obs, oldVal, newVal) -> {
            if (!newVal)
                actualizarZ0(linea, txtZ0);
        });
        addField("Z0 (p.u.):", 4, txtZ0);
        ...
        txtResistencia.focusedProperty().addListener((obs, oldVal, newVal) -> {
            if (!newVal)
                actualizarResistencia(linea, txtResistencia);
        });
        addField("Resistencia (p.u.):", 5, txtResistencia);
        ...
        txtYMedia.focusedProperty().addListener((obs, oldVal, newVal) -> {
            if (!newVal)
                actualizarYMedia(linea, txtYMedia);
        });
        addField("Y/2 (p.u.):", 6, txtYMedia);
