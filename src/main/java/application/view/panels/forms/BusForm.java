...
public class BusForm extends AbstractForm<Barras> {
  ...
  public TitledPane render(Barras barra) {
    ...
    TextField txtLogico =
        new TextField(barra.isBarraCompensacion() ? "Slack" : barra.getNombreLogico());
    ...
    barra.addPropertyChangeListener(
        evt -> {
          ...
          if ("nombreBarra".equals(prop) || "barraCompensacion".equals(prop)) {
            javafx.application.Platform.runLater(
                () ->
                    txtLogico.setText(
                        barra.isBarraCompensacion() ? "Slack" : barra.getNombreLogico()));
          }
        });
    ...
    // --- 1b. CONECTIVIDAD (Resumen) ---
    long numLineas =
        NetworkModel.getInstance().getLineas().stream()
            .filter(l -> l.getBarra1() == barra || l.getBarra2() == barra)
            .count();
    long numGens =
        NetworkModel.getInstance().getGeneradores().stream()
            .filter(g -> g.getBarra() == barra)
            .count();
    long numTrafos =
        NetworkModel.getInstance().getTransformadores().stream()
            .filter(t -> t.getBarra1() == barra || t.getBarra2() == barra)
            .count();

    VBox boxConectividad = new VBox(2);
    boxConectividad.getChildren().add(new Label("Líneas: " + numLineas));
    boxConectividad.getChildren().add(new Label("Generadores: " + numGens));
    boxConectividad.getChildren().add(new Label("Transformadores: " + numTrafos));

    addField("Conectados:", 5, boxConectividad);
