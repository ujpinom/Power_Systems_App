...
public class EcuacionesVoltajeYPotencia {
  ...
  public EcuacionesVoltajeYPotencia(
      List<Barras> barras, int numeroIteraciones, double factorAceleracion, double epsilon) {
    ...
    for (int i = 1; i < barras.size(); i++) {
      ...
      infoIteraciones[i].add(
          Complejo.polar2Cartesiano(
              barras.get(i).getVoltajePrefalla(), barras.get(i).getAnguloVoltajeBarra()));
    }
  }
  ...
  public List<Complejo>[] calcularFlujoPotencia(
      List<Generadores> generadores,
      List<Bancos> bancos,
      List<Carga> cargas,
      Complejo[][] matrizAdj) {
  ...
  private void calcularFlujoPotenciaHelper(
      List<Generadores> generadores,
      List<Bancos> bancos,
      List<Carga> cargas,
      Complejo[][] matrizAdj) {
    ...
    int objetosCumplenEpsilon =
        0; // Valida cuantos objetos han cumplido epsilon. Todos los objetos menos la barra de
    // tierra deben cumplir este criterio
    while (iteracionActual <= numeroIteraciones) {
      ...
      for (int i = 1; i < barras.size(); i++) {
        ...
        if (b.isBarraCompensacion()) {
          ...
          if (Math.abs(
                  infoIteraciones[i].get(infoIteraciones[i].size() - 1).modulo()
                      - infoIteraciones[i].get(infoIteraciones[i].size() - 2).modulo())
              <= epsilon) {

            if (Math.abs(
                    infoIteraciones[i].get(infoIteraciones[i].size() - 1).argumento()
                        - infoIteraciones[i].get(infoIteraciones[i].size() - 2).argumento())
                <= epsilon) objetosCumplenEpsilon++;
          }
  ...
  /**
   * Retorna el número de la iteración para la cual el algoritmo alcanza la convergencia según el
   * epsilón especificado.
   *
   * @return
   */
  ...
  private void acelerar(int i) {
    ...
    Complejo voltajeAcelerado =
        Complejo.suma(
            Complejo.producto(
                new Complejo(factorAceleracion, 0), Complejo.resta(voltajeActual, voltajeAnterior)),
            voltajeAnterior);
  ...
  private void calculoBarraPVCompensador(Barras b) {
    ...
    Complejo voltajeIteAnteriorConjugado =
        Complejo.conjugado(infoIteraciones[indexBarra].get(infoIteraciones[indexBarra].size() - 1));
    ...
    for (int i = 1; i < infoIteraciones.length; i++) {
      ...
      resultado =
          Complejo.suma(
              resultado,
              Complejo.producto(
                  matrizAdj[indexBarra][i], infoIteraciones[i].get(infoIteraciones[i].size() - 1)));
    }
    ...
    if ((potenciaImCalculada + potenciaReactivaCarga) > potenciaReactivaMaximaCompensador) {
      ...
      // Se trata la barra PV como barra PQ ya que el generador no es capaz de suplir la Q para
      // mantener el nivel de voltaje  especificado.
    ...
    try {
      ...
      if (violaLimites) {
        ...
        /**
         * Se corrige la magnitud del voltaje dado que es magnitud especificada en el problema de
         * flujo de potencia.
         */
        resultado =
            Complejo.cociente(
                Complejo.producto(
                    new Complejo(
                        infoIteraciones[indexBarra]
                            .get(infoIteraciones[indexBarra].size() - 1)
                            .modulo(),
                        0),
                    resultado),
                new Complejo(magResultado, 0));
  ...
  private void calculoBarrasPV(Barras b) {
    ...
    double potenciaReactivaMaximaGenerador =
        b.getGenerador().getMVarSalidaMax() / SPController.BASE_SISTEMA;

    double potenciaReactivaMinimaGenerador =
        b.getGenerador().getMVarSalidaMin() / SPController.BASE_SISTEMA;
  ...
  private void calculoBarrasPQ(Barras b) throws ExcepcionDivideCero {
    ...
    if (!b.containsCarga() && !b.isBarraFromPV2PQ()) {
      ...
      potenciaComplejaProgramada =
          Complejo.producto(
              new Complejo(-1, 0),
              new Complejo(
                  b.getCarga().getPotenciaActiva() / SPController.BASE_SISTEMA,
    ...
    try {
      ...
      List<Complejo> primeraSumatoria = primeraSumatoria(indexBarra, matrizAdj);
      Complejo resultado =
          Complejo.cociente(potenciaComplejaProgramada, voltajeIteAnteriorConjugado);
