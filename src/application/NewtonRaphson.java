...
public class NewtonRaphson implements Cloneable {
	...
	private List<Integer> indicesBarrasCompPV= new ArrayList<>(); // Almacena el indice de las barras que son de compensación o  PV.
...
//							,{new Complejo(),new Complejo(-5.169561,25.847809),new Complejo(),new Complejo(8.193267,-40.863838),new Complejo(-3.023705,15.118528)},
//							{new Complejo(),new Complejo(),new Complejo(-5.169561,25.847809),new Complejo(-3.023705,15.118528),new Complejo(8.193267,-40.863838)}};
//	
	...
	public void calcularFlujoPotencia(List<Generadores> generadores,List<Bancos> bancos,List<Carga> cargas
		...
		while(iteracionActual<=numeroIteraciones) {
			...
			int [] indicesAMantener= new int[jacobiana.length-dimensionMatrizjacobiana];  /// La solución del problema de flujos de potencia corresponden a estas barras +1.
			int contador=0;
			...
			for(int i=1;i<infoIteracionesAngulos.length;i++) {
				...
				if(barras.get(i).isBarraCompensacion()) {
					...
					if(Math.abs(infoIteracionesAngulos[i].get(angulos-1)-infoIteracionesAngulos[i].get(angulos-2))<=epsilon) {
					...
					if(Math.abs(infoIteracionesAngulos[i].get(angulos-1)-infoIteracionesAngulos[i].get(angulos-2))<=epsilon
					&& (Math.abs(infoIteracionesVoltajes[i].get(voltajes-1)-infoIteracionesVoltajes[i].get(voltajes-2))<=epsilon)) {	
						++cont;
		...
		for(int i=0;i<indicesBarras.length;i++) {
			...
			if(indicesBarras[i]<jacobiana.length/2) {
				...
				infoIteracionesAngulos[barra].add((anguloPrevio+errorParametroCalculado));
					...
					double voltajePrevio=infoIteracionesVoltajes[barra-(barras.size()-1)].get(infoIteracionesVoltajes[barra-(barras.size()-1)].size()-1);
...
//				angV_i= (angV_i*Math.PI)/180.0;
		...
		if(b.containsBanco()) {
		 ...
		 potenciaImaginariaCalculada=(-1)*infoIteracionesVoltajes[index].get(infoIteracionesVoltajes[index].size()-1)*
	 ...
	 for(int j=barras.size()-1;j<jacobiana.length;j++) {
		 ...
		 if(j-(barras.size()-1)!=i) {
			...
			jacobiana[i][j]=(-1)*jacobiana[i+barras.size()-1][j-(barras.size()-1)];
			 ...
			 jacobiana[i][j]= jacobiana[i+barras.size()-1][j-(barras.size()-1)]+2*infoIteracionesVoltajes[index].get(infoIteracionesVoltajes[index].size()-1)*
					 infoIteracionesVoltajes[index].get(infoIteracionesVoltajes[index].size()-1)*resis;
